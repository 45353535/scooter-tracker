...
public class TypeUtils {
    ...
    */
    ...
    public static List<KotlinType> getImmediateSupertypes(@NotNull KotlinType kotlinType) {
        ...
        Collection<KotlinType> collectionMo8664getSupertypes = kotlinType.getConstructor().mo8664getSupertypes();
        ArrayList arrayList = new ArrayList(collectionMo8664getSupertypes.size());
        Iterator<KotlinType> it = collectionMo8664getSupertypes.iterator();
        while (it.hasNext()) {
    ...
    public static TypeParameterDescriptor getTypeParameterDescriptorOrNull(@NotNull KotlinType kotlinType) {
        ...
        if (kotlinType.getConstructor().mo8663getDeclarationDescriptor() instanceof TypeParameterDescriptor) {
            return (TypeParameterDescriptor) kotlinType.getConstructor().mo8663getDeclarationDescriptor();
        }
        return null;
    }
    ...
    public static boolean hasNullableSuperType(@NotNull KotlinType kotlinType) {
        ...
        if (kotlinType.getConstructor().mo8663getDeclarationDescriptor() instanceof ClassDescriptor) {
            return false;
        }
    ...
    public static boolean isNullableType(@NotNull KotlinType kotlinType) {
        ...
        if (constructor instanceof IntersectionTypeConstructor) {
            Iterator<KotlinType> it = constructor.mo8664getSupertypes().iterator();
            while (it.hasNext()) {
                if (isNullableType(it.next())) {
                    return true;
                }
    ...
    public static KotlinType makeNullableAsSpecified(@NotNull KotlinType kotlinType, boolean z10) {
        ...
        UnwrappedType unwrappedTypeMakeNullableAsSpecified = kotlinType.unwrap().makeNullableAsSpecified(z10);
    ...
    private static boolean contains(@Nullable KotlinType kotlinType, @NotNull Function1<UnwrappedType, Boolean> function1, SmartSet<KotlinType> smartSet) {
        ...
        TypeConstructor constructor = kotlinType.getConstructor();
        if (constructor instanceof IntersectionTypeConstructor) {
            Iterator<KotlinType> it = ((IntersectionTypeConstructor) constructor).mo8664getSupertypes().iterator();
            while (it.hasNext()) {
