...
public final class RuntimeTypeMapper {
    ...
    public final JvmFunctionSignature mapSignature(@NotNull FunctionDescriptor possiblySubstitutedFunction) {
        ...
        if (constructorDescriptor.isPrimary()) {
            if (!Intrinsics.areEqual(jvmConstructorSignature.getName(), "constructor-impl") || !StringsKt.M(jvmConstructorSignature.getDesc(), ")V", false, 2, null)) {
            ...
            if (StringsKt.M(jvmConstructorSignature.getDesc(), ")V", false, 2, null)) {
                jvmConstructorSignature = JvmMemberSignature.Method.copy$default(jvmConstructorSignature, null, StringsKt.Q0(jvmConstructorSignature.getDesc(), ExifInterface.GPS_MEASUREMENT_INTERRUPTED) + jvmDescriptor, 1, null);
            } else if (!StringsKt.M(jvmConstructorSignature.getDesc(), jvmDescriptor, false, 2, null)) {
                throw new IllegalArgumentException(("Invalid signature: " + jvmConstructorSignature).toString());
            }
        }
        return new JvmFunctionSignature.KotlinFunction(jvmConstructorSignature);
