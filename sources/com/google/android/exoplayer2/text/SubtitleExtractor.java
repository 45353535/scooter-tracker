...
public class SubtitleExtractor implements Extractor {
    ...
    private void decode() throws IOException, DecoderException {
        try {
            ...
            for (int i10 = 0; i10 < subtitleOutputBufferDequeueOutputBuffer.getEventTimeCount(); i10++) {
                byte[] bArrEncode = this.cueEncoder.encode(subtitleOutputBufferDequeueOutputBuffer.getCues(subtitleOutputBufferDequeueOutputBuffer.getEventTime(i10)));
    ...
    private boolean readFromInput(ExtractorInput extractorInput) throws IOException {
        ...
        if (iCapacity == i10) {
            this.subtitleData.ensureCapacity(i10 + 1024);
        }
        int i11 = extractorInput.read(this.subtitleData.getData(), this.bytesRead, this.subtitleData.capacity() - this.bytesRead);
        if (i11 != -1) {
            this.bytesRead += i11;
        }
    ...
    private void writeToOutput() {
        ...
        for (int iBinarySearchFloor = j10 == -9223372036854775807L ? 0 : Util.binarySearchFloor((List<? extends Comparable<? super Long>>) this.timestamps, Long.valueOf(j10), true, true); iBinarySearchFloor < this.samples.size(); iBinarySearchFloor++) {
