...
public abstract class AbstractC3445nz {
    ...
    public static int A00(int expectedSize) {
        if (expectedSize < 3) {
            AbstractC3309ll.A00(expectedSize, A07(2, 12, 124));
    ...
    public static <K, V> Map.Entry<K, V> A01(@ParametricNullness K key, @ParametricNullness V value) {
        return new C1965An(key, value);
    }

    public static <K, V> Iterator<K> A02(Iterator<Map.Entry<K, V>> entryIterator) {
        return new C1958Ag(entryIterator);
    }

    public static <K, V> Iterator<V> A03(Iterator<Map.Entry<K, V>> entryIterator) {
        return new C1957Af(entryIterator);
    }

    public static <V> InterfaceC3142ip<Map.Entry<?, V>, V> A04() {
        EnumC1956Ae enumC1956Ae = EnumC1956Ae.A03;
        if (A01[6].charAt(13) == 's') {
            throw new RuntimeException();
        }
    ...
    public static String A08(Map<?, ?> map) {
        StringBuilder sbAppend = AbstractC3314lq.A01(map.size()).append('{');
        boolean z10 = true;
        for (Map.Entry<?, ?> entry : map.entrySet()) {
            ...
            z10 = false;
            sbAppend.append(entry.getKey()).append(D5.T).append(entry.getValue());
        }
    ...
    public static boolean A0A(@CheckForNull Map<?, ?> map, Object value) {
        return AbstractC3406nL.A0E(A03(map.entrySet().iterator()), value);
    }
