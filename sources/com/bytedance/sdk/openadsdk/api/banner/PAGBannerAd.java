...
public abstract class PAGBannerAd implements PAGClientBidding, PangleAd {
    public static void loadAd(@NonNull String str, @NonNull PAGBannerRequest pAGBannerRequest, @NonNull PAGBannerAdLoadListener pAGBannerAdLoadListener) {
        ISDKTypeFactory iSDKTypeFactoryQdl = mzz.qdl(pAGBannerAdLoadListener);
        if (iSDKTypeFactoryQdl != null) {
            iSDKTypeFactoryQdl.createADTypeLoaderFactory(str).createBannerAdLoader().loadAd(str, pAGBannerRequest, pAGBannerAdLoadListener);
        }
    }
