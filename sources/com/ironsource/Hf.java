package com.ironsource;

import android.annotation.SuppressLint;
import android.os.Handler;
import android.webkit.WebResourceError;
import android.webkit.WebResourceRequest;
import android.webkit.WebSettings;
import android.webkit.WebView;
import android.webkit.WebViewClient;
import com.ironsource.Hf;
import com.ironsource.mediationsdk.testSuite.TestSuiteActivity;
import com.unity3d.mediation.LevelPlay;
import java.util.List;
import kotlin.collections.CollectionsKt;
import kotlin.jvm.internal.Intrinsics;
import kotlin.text.StringsKt;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;
import org.json.JSONException;
import org.json.JSONObject;

/* JADX INFO: loaded from: classes8.dex */
public final class Hf implements InterfaceC4625xf {

    /* JADX INFO: renamed from: a, reason: collision with root package name */
    @NotNull
    private final Handler f41063a;

    /* JADX INFO: renamed from: b, reason: collision with root package name */
    @Nullable
    private WebView f41064b;

    /* JADX INFO: renamed from: c, reason: collision with root package name */
    @NotNull
    private final WebView f41065c;

    /* JADX INFO: renamed from: d, reason: collision with root package name */
    @NotNull
    private final JSONObject f41066d;

    /* JADX INFO: renamed from: e, reason: collision with root package name */
    private boolean f41067e;

    /* JADX INFO: renamed from: f, reason: collision with root package name */
    private boolean f41068f;

    public static final class a extends WebViewClient {

        /* JADX INFO: renamed from: b, reason: collision with root package name */
        final /* synthetic */ String f41070b;

        a(String str) {
            this.f41070b = str;
        }

        @Override // android.webkit.WebViewClient
        public void onPageFinished(@Nullable WebView webView, @Nullable String str) throws JSONException {
            if (!Hf.this.f41067e && str != null && StringsKt.a0(str, String.valueOf(this.f41070b), false, 2, null)) {
                Hf hf2 = Hf.this;
                List<? extends Object> listListOf = CollectionsKt.listOf(hf2.f41066d);
                Intrinsics.checkNotNull(listListOf, "null cannot be cast to non-null type kotlin.collections.List<kotlin.Any?>");
                hf2.a(hf2.a("onDataReady", listListOf));
                C4540sf.f45416a.d();
                Hf.this.f41067e = true;
            }
            super.onPageFinished(webView, str);
        }

        @Override // android.webkit.WebViewClient
        public void onReceivedError(@NotNull WebView view, @NotNull WebResourceRequest request, @NotNull WebResourceError error) throws JSONException {
            Intrinsics.checkNotNullParameter(view, "view");
            Intrinsics.checkNotNullParameter(request, "request");
            Intrinsics.checkNotNullParameter(error, "error");
            C4540sf.f45416a.a(error.toString());
            Bf.f40456a.b("got error during TestSuite web controller loading " + error);
            super.onReceivedError(view, request, error);
        }
    }

    public static final class b implements S8 {

        /* JADX INFO: renamed from: b, reason: collision with root package name */
        final /* synthetic */ S8 f41072b;

        b(S8 s82) {
            this.f41072b = s82;
        }

        /* JADX INFO: Access modifiers changed from: private */
        public static final void a(b this$0) {
            Intrinsics.checkNotNullParameter(this$0, "this$0");
            this$0.onUIReady();
        }

        @Override // com.ironsource.S8
        public void onClosed() {
            this.f41072b.onClosed();
        }

        @Override // com.ironsource.S8
        public void onUIReady() {
            if (Hf.this.f41068f) {
                this.f41072b.onUIReady();
            } else {
                Hf.this.f41063a.postDelayed(new Runnable() { // from class: com.ironsource.di
                    @Override // java.lang.Runnable
                    public final void run() {
                        Hf.b.a(this.f42927b);
                    }
                }, 500L);
            }
        }
    }

    public Hf(@NotNull TestSuiteActivity activity, @NotNull S8 uiLifeCycleListener, @NotNull JSONObject jsonData, @Nullable String str) {
        Intrinsics.checkNotNullParameter(activity, "activity");
        Intrinsics.checkNotNullParameter(uiLifeCycleListener, "uiLifeCycleListener");
        Intrinsics.checkNotNullParameter(jsonData, "jsonData");
        this.f41063a = new Handler(activity.getMainLooper());
        this.f41064b = new WebView(activity);
        this.f41065c = new WebView(activity);
        this.f41066d = jsonData;
        a(activity, uiLifeCycleListener, str);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final void e(Hf this$0) {
        Intrinsics.checkNotNullParameter(this$0, "this$0");
        this$0.f41068f = true;
    }

    @NotNull
    public final WebView c() {
        return this.f41065c;
    }

    @Nullable
    public final WebView d() {
        return this.f41064b;
    }

    @SuppressLint({"SetJavaScriptEnabled", "AddJavascriptInterface"})
    private final void a(TestSuiteActivity testSuiteActivity, final S8 s82, String str) {
        WebView webView = this.f41064b;
        WebSettings settings = webView != null ? webView.getSettings() : null;
        if (settings != null) {
            settings.setJavaScriptEnabled(true);
        }
        this.f41065c.getSettings().setJavaScriptEnabled(true);
        this.f41065c.addJavascriptInterface(new C4608wf(new C4507qf(testSuiteActivity, this.f41063a), new b(s82), this), "TestSuiteBridge");
        this.f41065c.setWebViewClient(new a(str));
        if (str != null && str.length() != 0) {
            this.f41065c.loadUrl(str);
        }
        WebView webView2 = this.f41064b;
        if (webView2 != null) {
            webView2.loadUrl("javascript:document.write(atob(''))");
        }
        this.f41063a.postDelayed(new Runnable() { // from class: com.ironsource.bi
            @Override // java.lang.Runnable
            public final void run() {
                Hf.e(this.f42779b);
            }
        }, 2000L);
        this.f41063a.postDelayed(new Runnable() { // from class: com.ironsource.ci
            @Override // java.lang.Runnable
            public final void run() {
                Hf.a(s82);
            }
        }, 5000L);
    }

    public final void b() {
        WebView webView = this.f41064b;
        if (webView != null) {
            webView.destroy();
            this.f41064b = null;
        }
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final void a(S8 uiLifeCycleListener) {
        Intrinsics.checkNotNullParameter(uiLifeCycleListener, "$uiLifeCycleListener");
        uiLifeCycleListener.onUIReady();
    }

    @Override // com.ironsource.InterfaceC4625xf
    public void a(@NotNull String callbackName, @NotNull LevelPlay.AdFormat adFormat, @NotNull List<? extends Object> args) {
        Intrinsics.checkNotNullParameter(callbackName, "callbackName");
        Intrinsics.checkNotNullParameter(adFormat, "adFormat");
        Intrinsics.checkNotNullParameter(args, "args");
        if (this.f41067e) {
            a(a("onCallbackReceived", CollectionsKt.listOf(callbackName, adFormat.getValue(), args)));
        }
    }

    public final void a(@NotNull final String script) {
        Intrinsics.checkNotNullParameter(script, "script");
        this.f41063a.post(new Runnable() { // from class: com.ironsource.ai
            @Override // java.lang.Runnable
            public final void run() {
                Hf.a(this.f42438b, script);
            }
        });
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final void a(Hf this$0, String script) {
        Intrinsics.checkNotNullParameter(this$0, "this$0");
        Intrinsics.checkNotNullParameter(script, "$script");
        this$0.f41065c.loadUrl("javascript:" + script);
    }

    @NotNull
    public final String a(@NotNull String methodName, @NotNull List<? extends Object> methodArgs) {
        Intrinsics.checkNotNullParameter(methodName, "methodName");
        Intrinsics.checkNotNullParameter(methodArgs, "methodArgs");
        String str = "TestSuiteJs." + methodName;
        String strB = Gf.f41012a.b(methodArgs);
        if (strB.length() > 0) {
            return str + "(" + strB + ")";
        }
        return str + "()";
    }

    public final void a() {
        b();
        this.f41065c.destroy();
    }
}
