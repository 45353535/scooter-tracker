...
public final class DtbDeviceData {
    ...
    private void buildDeviceParams() {
        ...
        String str = this.f7405ua;
        if (str != null) {
            this.deviceParams.put(N6.f41375d0, str);
        }
    ...
    private void getUserAgent() {
        try {
            this.f7405ua = WebSettings.getDefaultUserAgent(AdRegistration.getContext());
        } catch (Exception unused) {
            DtbLog.info("Unable to Get User Agent, Setting it to default");
            this.f7405ua = "Android";
        }
    }

    public HashMap<String, Object> getDeviceParams() {
        if (!this.deviceParams.containsKey(N6.f41375d0) || (this.deviceParams.containsKey(N6.f41375d0) && this.deviceParams.get(N6.f41375d0).equals("Android"))) {
            getUserAgent();
            buildDeviceParams();
        }
