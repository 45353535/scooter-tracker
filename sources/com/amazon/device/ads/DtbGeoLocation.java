...
class DtbGeoLocation {
    ...
    @Nullable
    Location getLocationInComputeMode(float f10) {
    ...
    @Nullable
    Location getLocationInFixMode(float f10) {
        Context context = AdRegistration.getContext();
        if (f10 > (hasLocationFinePermission(context) ? 1.0f : hasLocationCoarsePermission(context) ? ACCURACY_IN_METERS_COARSE_PERMISSION_FIX_MODE : 0.0f)) {
            return null;
        }
        return getLocation();
    }
