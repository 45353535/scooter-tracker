...
final class WavHeaderReader {
    ...
    public static WavHeader peek(ExtractorInput extractorInput) throws InterruptedException, IOException {
        ...
        if (ChunkHeader.peek(extractorInput, parsableByteArray).f50059id != Util.getIntegerCodeForString("RIFF")) {
            return null;
        }
        int pcmEncoding = 0;
        ...
        while (chunkHeaderPeek.f50059id != Util.getIntegerCodeForString("fmt ")) {
    ...
    public static void skipToData(ExtractorInput extractorInput, WavHeader wavHeader) throws InterruptedException, IOException {
        ...
        ChunkHeader chunkHeaderPeek = ChunkHeader.peek(extractorInput, parsableByteArray);
        while (chunkHeaderPeek.f50059id != Util.getIntegerCodeForString("data")) {
            Log.w(TAG, "Ignoring unknown WAV chunk: " + chunkHeaderPeek.f50059id);
            long j10 = chunkHeaderPeek.size + 8;
            if (chunkHeaderPeek.f50059id == Util.getIntegerCodeForString("RIFF")) {
                j10 = 12;
            }
            if (j10 > SieveCacheKt.NodeLinkMask) {
                throw new ParserException("Chunk is too large (~2GB+) to skip; id: " + chunkHeaderPeek.f50059id);
            }
