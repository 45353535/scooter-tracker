...
final class ExoPlayerImplInternal implements Handler.Callback, MediaPeriod.Callback, TrackSelector.InvalidationListener, MediaSource.SourceInfoRefreshListener, DefaultMediaClock.PlaybackParameterListener, PlayerMessage.Sender {
    ...
    private boolean isTimelineReady() {
        MediaPeriodHolder playingPeriod = this.queue.getPlayingPeriod();
        long j10 = playingPeriod.info.durationUs;
        if (j10 == -9223372036854775807L || this.playbackInfo.positionUs < j10) {
            return true;
        }
        MediaPeriodHolder mediaPeriodHolder = playingPeriod.next;
        if (mediaPeriodHolder != null) {
            return mediaPeriodHolder.prepared || mediaPeriodHolder.info.f50055id.isAd();
        }
        return false;
    }
    ...
    */
    ...
    private void maybeUpdateLoadingPeriod() throws IOException {
        ...
        if (this.queue.shouldLoadNextMediaPeriod()) {
            ...
            this.queue.enqueueNextMediaPeriod(this.rendererCapabilities, this.trackSelector, this.loadControl.getAllocator(), this.mediaSource, this.playbackInfo.timeline.getPeriod(nextMediaPeriodInfo.f50055id.periodIndex, this.period, true).uid, nextMediaPeriodInfo).prepare(this, nextMediaPeriodInfo.startPositionUs);
            setIsLoading(true);
        }
    }
    ...
    private void seekToCurrentPosition(boolean z10) throws ExoPlaybackException {
        MediaSource.MediaPeriodId mediaPeriodId = this.queue.getPlayingPeriod().info.f50055id;
    ...
    */
    ...
    private boolean shouldKeepPeriodHolder(MediaSource.MediaPeriodId mediaPeriodId, long j10, MediaPeriodHolder mediaPeriodHolder) {
        if (!mediaPeriodId.equals(mediaPeriodHolder.info.f50055id) || !mediaPeriodHolder.prepared) {
            return false;
        }
        this.playbackInfo.timeline.getPeriod(mediaPeriodHolder.info.f50055id.periodIndex, this.period);
    ...
    private void updatePeriods() throws IOException, ExoPlaybackException {
        ...
        while (this.playWhenReady && playingPeriod != readingPeriod && this.rendererPositionUs >= playingPeriod.next.rendererPositionOffsetUs) {
            ...
            this.playbackInfo = playbackInfo.fromNewPosition(mediaPeriodInfo.f50055id, mediaPeriodInfo.startPositionUs, mediaPeriodInfo.contentPositionUs);
