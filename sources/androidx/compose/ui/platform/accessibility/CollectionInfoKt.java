    ...
    private static final boolean calculateIfHorizontallyStacked(List<SemanticsNode> list) {
        ...
        if (list.size() == 0 || list.size() == 1) {
            ...
            while (i10 < lastIndex) {
                ...
                listEmptyList.add(Offset.m1360boximpl(OffsetKt.Offset(Math.abs(Offset.m1371getXimpl(semanticsNode4.getBoundsInRoot().m1401getCenterF1C5BW0()) - Offset.m1371getXimpl(semanticsNode3.getBoundsInRoot().m1401getCenterF1C5BW0())), Math.abs(Offset.m1372getYimpl(semanticsNode4.getBoundsInRoot().m1401getCenterF1C5BW0()) - Offset.m1372getYimpl(semanticsNode3.getBoundsInRoot().m1401getCenterF1C5BW0())))));
        ...
        if (listEmptyList.size() == 1) {
            ...
            if (1 <= lastIndex2) {
                int i11 = 1;
                while (true) {
                    objFirst = Offset.m1360boximpl(Offset.m1376plusMKHz9U(((Offset) objFirst).getPackedValue(), ((Offset) listEmptyList.get(i11)).getPackedValue()));
                    if (i11 == lastIndex2) {
                        break;
                    }
                    i11++;
        ...
        return Offset.m1362component2impl(packedValue) < Offset.m1361component1impl(packedValue);
    }
    ...
    public static final void setCollectionItemInfo(@NotNull SemanticsNode node, @NotNull AccessibilityNodeInfoCompat info) {
        ...
        if ((collectionInfo == null || !isLazyCollection(collectionInfo)) && node.getConfig().contains(semanticsProperties.getSelected())) {
            ...
            for (int i11 = 0; i11 < size; i11++) {
                SemanticsNode semanticsNode = replacedChildren$ui_release.get(i11);
                if (semanticsNode.getConfig().contains(SemanticsProperties.INSTANCE.getSelected())) {
                    arrayList.add(semanticsNode);
            ...
            boolean zCalculateIfHorizontallyStacked = calculateIfHorizontallyStacked(arrayList);
            AccessibilityNodeInfoCompat.CollectionItemInfoCompat collectionItemInfoCompatObtain = AccessibilityNodeInfoCompat.CollectionItemInfoCompat.obtain(zCalculateIfHorizontallyStacked ? 0 : i10, 1, zCalculateIfHorizontallyStacked ? i10 : 0, 1, false, ((Boolean) node.getConfig().getOrElse(SemanticsProperties.INSTANCE.getSelected(), new Function0<Boolean>() { // from class: androidx.compose.ui.platform.accessibility.CollectionInfoKt$setCollectionItemInfo$itemInfo$1
