...
public final class TextLayoutHelperKt {
    ...
    public static final boolean m790canReuse7_7YC6M(@NotNull TextLayoutResult canReuse, @NotNull AnnotatedString text, @NotNull TextStyle style, @NotNull List<AnnotatedString.Range<Placeholder>> placeholders, int i10, boolean z10, int i11, @NotNull Density density, @NotNull LayoutDirection layoutDirection, @NotNull FontFamily.Resolver fontFamilyResolver, long j10) {
        ...
        if (canReuse.getMultiParagraph().getIntrinsics().getHasStaleResolvedFonts() || !Intrinsics.areEqual(layoutInput.getText(), text) || !layoutInput.getStyle().hasSameLayoutAffectingAttributes(style) || !Intrinsics.areEqual(layoutInput.getPlaceholders(), placeholders) || layoutInput.getMaxLines() != i10 || layoutInput.getSoftWrap() != z10 || !TextOverflow.m3897equalsimpl0(layoutInput.getOverflow(), i11) || !Intrinsics.areEqual(layoutInput.getDensity(), density) || layoutInput.getLayoutDirection() != layoutDirection || !Intrinsics.areEqual(layoutInput.getFontFamilyResolver(), fontFamilyResolver) || Constraints.m3921getMinWidthimpl(j10) != Constraints.m3921getMinWidthimpl(layoutInput.getConstraints())) {
            return false;
        }
        if (z10 || TextOverflow.m3897equalsimpl0(i11, TextOverflow.INSTANCE.m3905getEllipsisgIe3tQ8())) {
            return Constraints.m3919getMaxWidthimpl(j10) == Constraints.m3919getMaxWidthimpl(layoutInput.getConstraints()) && Constraints.m3918getMaxHeightimpl(j10) == Constraints.m3918getMaxHeightimpl(layoutInput.getConstraints());
        }
        return true;
    }
}
