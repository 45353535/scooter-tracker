...
final class LazyStaggeredGridMeasureProvider {
    ...
    public LazyStaggeredGridMeasureProvider(boolean z10, @NotNull LazyLayoutItemProvider itemProvider, @NotNull LazyLayoutMeasureScope measureScope, @NotNull int[] resolvedSlotSums, int i10, @NotNull MeasuredItemFactory measuredItemFactory) {
    ...
    private final long m641childConstraintsJhjzzOo(int slot, int span) {
        int i10 = (this.resolvedSlotSums[(slot + span) - 1] - (slot == 0 ? 0 : this.resolvedSlotSums[slot - 1])) + (this.crossAxisSpacing * (span - 1));
        return this.isVertical ? Constraints.INSTANCE.m3929fixedWidthOenEA2s(i10) : Constraints.INSTANCE.m3928fixedHeightOenEA2s(i10);
    }

    @NotNull
    /* JADX INFO: renamed from: getAndMeasure-jy6DScQ, reason: not valid java name */
    public final LazyStaggeredGridMeasuredItem m642getAndMeasurejy6DScQ(int index, long span) {
        int i10 = (int) (span >> 32);
        int i11 = ((int) (span & 4294967295L)) - i10;
        return this.measuredItemFactory.createItem(index, i10, i11, this.itemProvider.getKey(index), this.measureScope.mo619measure0kLqBqw(index, m641childConstraintsJhjzzOo(i10, i11)));
    }
}
