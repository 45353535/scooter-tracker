...
public final class AbsoluteCutCornerShape extends CornerBasedShape {
    ...
    public Outline mo673createOutlineLjSzlW0(long size, float topStart, float topEnd, float bottomEnd, float bottomStart, @NotNull LayoutDirection layoutDirection) {
        Intrinsics.checkNotNullParameter(layoutDirection, "layoutDirection");
        if (topStart + topEnd + bottomStart + bottomEnd == 0.0f) {
            return new Outline.Rectangle(SizeKt.m1461toRectuvyYCjk(size));
        }
        Path Path = AndroidPath_androidKt.Path();
        Path.moveTo(0.0f, topStart);
        Path.lineTo(topStart, 0.0f);
        Path.lineTo(Size.m1440getWidthimpl(size) - topEnd, 0.0f);
        Path.lineTo(Size.m1440getWidthimpl(size), topEnd);
        Path.lineTo(Size.m1440getWidthimpl(size), Size.m1437getHeightimpl(size) - bottomEnd);
        Path.lineTo(Size.m1440getWidthimpl(size) - bottomEnd, Size.m1437getHeightimpl(size));
        Path.lineTo(bottomStart, Size.m1437getHeightimpl(size));
        Path.lineTo(0.0f, Size.m1437getHeightimpl(size) - bottomStart);
        Path.close();
