...
final /* data */ class BottomAppBarCutoutShape implements Shape {
    ...
    private final void addCutoutShape(Path path, LayoutDirection layoutDirection, Density density) {
        float fMo308toPx0680j_4 = density.mo308toPx0680j_4(AppBarKt.BottomAppBarCutoutOffset);
        float f10 = 2 * fMo308toPx0680j_4;
        long jSize = SizeKt.Size(this.fabPlacement.getWidth() + f10, this.fabPlacement.getHeight() + f10);
        float left = this.fabPlacement.getLeft() - fMo308toPx0680j_4;
        float fM1440getWidthimpl = left + Size.m1440getWidthimpl(jSize);
        float fM1437getHeightimpl = Size.m1437getHeightimpl(jSize) / 2.0f;
        OutlineKt.addOutline(path, this.cutoutShape.mo212createOutlinePq9zytI(jSize, layoutDirection, density));
        path.mo1507translatek4lQ0M(OffsetKt.Offset(left, -fM1437getHeightimpl));
        if (Intrinsics.areEqual(this.cutoutShape, RoundedCornerShapeKt.getCircleShape())) {
            addRoundedEdges(path, left, fM1440getWidthimpl, fM1437getHeightimpl, density.mo308toPx0680j_4(AppBarKt.BottomAppBarRoundedEdgeRadius), 0.0f);
        }
    }
    ...
    public Outline mo212createOutlinePq9zytI(long size, @NotNull LayoutDirection layoutDirection, @NotNull Density density) {
        Intrinsics.checkNotNullParameter(layoutDirection, "layoutDirection");
        Intrinsics.checkNotNullParameter(density, "density");
        Path Path = AndroidPath_androidKt.Path();
        Path.addRect(new Rect(0.0f, 0.0f, Size.m1440getWidthimpl(size), Size.m1437getHeightimpl(size)));
        Path Path2 = AndroidPath_androidKt.Path();
        addCutoutShape(Path2, layoutDirection, density);
        Path2.mo1505opN5in7k0(Path, Path2, PathOperation.INSTANCE.m1866getDifferenceb3I0S0c());
