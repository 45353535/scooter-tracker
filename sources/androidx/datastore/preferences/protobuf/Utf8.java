...
final class Utf8 {
        ...
        final void encodeUtf8Default(String str, ByteBuffer byteBuffer) {
            ...
            while (i10 < length) {
                try {
                    ...
                    throw new ArrayIndexOutOfBoundsException("Failed writing " + str.charAt(i10) + " at index " + (byteBuffer.position() + Math.max(i10, (iPosition - byteBuffer.position()) + 1)));
                }
            }
            if (i10 == length) {
                Java8Compatibility.position(byteBuffer, iPosition + i10);
                return;
            }
            iPosition += i10;
            while (i10 < length) {
    ...
    static int encode(String str, byte[] bArr, int i10, int i11) {
    ...
    static int encodedLength(String str) {
        ...
        while (true) {
            if (i10 < length) {
                char cCharAt = str.charAt(i10);
                if (cCharAt >= 2048) {
                    iEncodedLengthGeneral += encodedLengthGeneral(str, i10);
                    break;
                }
    ...
    static final class UnsafeProcessor extends Processor {
        ...
        String decodeUtf8(byte[] bArr, int i10, int i11) throws InvalidProtocolBufferException {
            ...
            if (str.indexOf(com.applovin.shadow.okio.Utf8.REPLACEMENT_CODE_POINT) >= 0 && !Arrays.equals(str.getBytes(charset), Arrays.copyOfRange(bArr, i10, i11 + i10))) {
                throw InvalidProtocolBufferException.invalidUtf8();
            }
            return str;
        }
