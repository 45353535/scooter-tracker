...
public class MediaCodecAudioRenderer extends MediaCodecRenderer implements MediaClock {
    ...
    protected void onOutputFormatChanged(Format format, @Nullable MediaFormat mediaFormat) throws ExoPlaybackException {
        ...
        if (format2 != null) {
            ...
            Format formatBuild = new Format.Builder().setSampleMimeType("audio/raw").setPcmEncoding("audio/raw".equals(format.sampleMimeType) ? format.pcmEncoding : (Util.SDK_INT < 24 || !mediaFormat.containsKey("pcm-encoding")) ? mediaFormat.containsKey(VIVO_BITS_PER_SAMPLE_KEY) ? Util.getPcmEncoding(mediaFormat.getInteger(VIVO_BITS_PER_SAMPLE_KEY)) : 2 : mediaFormat.getInteger("pcm-encoding")).setEncoderDelay(format.encoderDelay).setEncoderPadding(format.encoderPadding).setMetadata(format.metadata).setCustomData(format.customData).setId(format.f4631id).setLabel(format.label).setLabels(format.labels).setLanguage(format.language).setSelectionFlags(format.selectionFlags).setRoleFlags(format.roleFlags).setChannelCount(mediaFormat.getInteger("channel-count")).setSampleRate(mediaFormat.getInteger("sample-rate")).build();
    ...
    protected int supportsFormat(MediaCodecSelector mediaCodecSelector, Format format) throws MediaCodecUtil.DecoderQueryException {
        ...
        if (!zSupportsFormatDrm || (z12 && MediaCodecUtil.getDecryptOnlyDecoderInfo() == null)) {
            ...
            if (this.audioSink.supportsFormat(format)) {
                return v2.e(4, 8, i11, audioOffloadSupport);
            }
        ...
        return v2.g(i14, i12, i11, mediaCodecInfo.hardwareAccelerated ? 64 : 0, z10 ? 128 : 0, i10);
    }
    ...
    private static List<MediaCodecInfo> getDecoderInfos(MediaCodecSelector mediaCodecSelector, Format format, boolean z10, AudioSink audioSink) throws MediaCodecUtil.DecoderQueryException {
        ...
        return MediaCodecUtil.getDecoderInfosSoftMatch(mediaCodecSelector, format, z10, false);
    }
    ...
    public MediaCodecAudioRenderer(Context context, MediaCodecSelector mediaCodecSelector, boolean z10, @Nullable Handler handler, @Nullable AudioRendererEventListener audioRendererEventListener, AudioSink audioSink) {
        this(context, androidx.media3.exoplayer.mediacodec.h.a(context), mediaCodecSelector, z10, handler, audioRendererEventListener, audioSink);
    }
