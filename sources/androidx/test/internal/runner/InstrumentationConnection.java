...
public class InstrumentationConnection {
    ...
    static class IncomingHandler extends Handler {
        ...
        private void clientsRegistrationFromBundle(Bundle bundle, boolean z10) {
        ...
        private void sendMessageToOtherInstr(int i10, Bundle bundle) {
            LogUtil.logDebugWithProcess(InstrumentationConnection.TAG, "sendMessageToOtherInstr() called with: what = [%s], data = [%s]", Integer.valueOf(i10), bundle);
            Iterator<Messenger> it = this.otherInstrumentations.iterator();
            while (it.hasNext()) {
                sendMessageWithReply(it.next(), i10, bundle);
            }
        }

        private void sendMessageWithReply(Messenger messenger, int i10, Bundle bundle) {
            LogUtil.logDebugWithProcess(InstrumentationConnection.TAG, "sendMessageWithReply type: " + i10 + " called", new Object[0]);
            Message messageObtainMessage = obtainMessage(i10);
            ...
            try {
                ...
                Log.w(InstrumentationConnection.TAG, "The remote process is terminated unexpectedly", e10);
        ...
        public void handleMessage(Message message) {
            switch (message.what) {
                ...
                case 12:
                    ...
                    break;
                default:
                    Log.w(InstrumentationConnection.TAG, "Unknown message code received: " + message.what);
    ...
    public synchronized void registerClient(String str, Messenger messenger) {
        ...
        Log.i(TAG, "Register client of type: " + str);
        Bundle bundle = new Bundle();
    ...
    public synchronized void requestRemoteInstancesActivityCleanup() {
        ...
        try {
            ...
            Log.e(TAG, "Interrupted while waiting for response from message with id: " + uuidRandomUUID, e10);
            incomingHandler = this.incomingHandler;
        }
    ...
    public synchronized void unregisterClient(String str, Messenger messenger) {
        Checks.checkState(this.incomingHandler != null, "Instrumentation Connection in not yet initialized");
        Log.i(TAG, "Unregister client of type: " + str);
        Bundle bundle = new Bundle();
        bundle.putString(BUNDLE_KEY_CLIENT_TYPE, str);
        bundle.putParcelable(BUNDLE_KEY_CLIENT_MESSENGER, messenger);
