package fd;

import android.content.Context;
import android.util.AttributeSet;
import android.view.View;
import android.widget.FrameLayout;
import android.widget.ImageView;
import androidx.annotation.Nullable;
import fd.b;
import pd.f0;
import pd.i0;

/* JADX INFO: loaded from: classes3.dex */
public class f extends FrameLayout {

    /* JADX INFO: renamed from: b, reason: collision with root package name */
    final fd.b f72003b;

    /* JADX INFO: renamed from: c, reason: collision with root package name */
    final ImageView f72004c;

    private final class b implements b.d {
        private b() {
        }

        @Override // fd.b.d
        public void a() {
            i0.g(f.this.f72003b);
            i0.n(f.this.f72004c);
        }
    }

    public interface c {
    }

    public f(Context context) {
        this(context, null, 0);
    }

    private ImageView c(Context context) {
        ImageView imageView = new ImageView(context);
        imageView.setImageBitmap(ic.k.c(""));
        imageView.setOnClickListener(new View.OnClickListener() { // from class: fd.e
            @Override // android.view.View.OnClickListener
            public final void onClick(View view) {
                this.f72002b.e(view);
            }
        });
        imageView.setColorFilter(ic.k.f74287c);
        return imageView;
    }

    /* JADX INFO: Access modifiers changed from: private */
    public /* synthetic */ void e(View view) {
        g();
    }

    private fd.b f(Context context) {
        fd.b bVar = new fd.b(context);
        bVar.setOnClickListener(new View.OnClickListener() { // from class: fd.d
            @Override // android.view.View.OnClickListener
            public final void onClick(View view) {
                this.f72001b.h(view);
            }
        });
        bVar.setListener(new b());
        return bVar;
    }

    /* JADX INFO: Access modifiers changed from: private */
    public /* synthetic */ void h(View view) {
        g();
    }

    private b9.c i(Context context) {
        b9.c cVar = new b9.c(context);
        cVar.setStyle(new b9.k().g(b9.a.f5852p));
        return cVar;
    }

    public void d() {
        i0.g(this);
        i0.g(this.f72003b);
        i0.g(this.f72004c);
    }

    public void j() {
        i0.n(this);
        i0.g(this.f72003b);
        i0.g(this.f72004c);
    }

    public f(Context context, AttributeSet attributeSet, int i10) {
        super(context, attributeSet, i10);
        ImageView imageViewC = c(context);
        this.f72004c = imageViewC;
        fd.b bVarF = f(context);
        this.f72003b = bVarF;
        b9.c cVarI = i(context);
        int iG = f0.g(getContext(), 48.0f);
        int iG2 = f0.g(getContext(), 30.0f);
        addView(cVarI, new FrameLayout.LayoutParams(iG, iG, 17));
        addView(bVarF, new FrameLayout.LayoutParams(iG2, iG2, 8388661));
        addView(imageViewC, new FrameLayout.LayoutParams(iG2, iG2, 8388661));
    }

    private void g() {
    }

    public void setListener(@Nullable c cVar) {
    }
}
