package yads;

import android.content.Context;
import android.view.View;
import android.view.Window;
import android.webkit.WebView;
import android.widget.ImageView;
import android.widget.LinearLayout;
import android.widget.ProgressBar;
import android.widget.RelativeLayout;
import android.widget.TextView;
import com.ironsource.C4240b4;

/* JADX INFO: loaded from: classes4.dex */
public final class t1 implements h2, t3 {

    /* JADX INFO: renamed from: a, reason: collision with root package name */
    public final Context f116009a;

    /* JADX INFO: renamed from: b, reason: collision with root package name */
    public final RelativeLayout f116010b;

    /* JADX INFO: renamed from: c, reason: collision with root package name */
    public final f2 f116011c;

    /* JADX INFO: renamed from: d, reason: collision with root package name */
    public final Window f116012d;

    /* JADX INFO: renamed from: e, reason: collision with root package name */
    public final String f116013e;

    /* JADX INFO: renamed from: f, reason: collision with root package name */
    public final u3 f116014f;

    /* JADX INFO: renamed from: g, reason: collision with root package name */
    public final LinearLayout f116015g;

    /* JADX INFO: renamed from: h, reason: collision with root package name */
    public final TextView f116016h;

    /* JADX INFO: renamed from: i, reason: collision with root package name */
    public final ProgressBar f116017i;

    /* JADX INFO: renamed from: j, reason: collision with root package name */
    public final r83 f116018j;

    public t1(Context context, RelativeLayout relativeLayout, q2 q2Var, Window window, String str, u3 u3Var, LinearLayout linearLayout, TextView textView, ProgressBar progressBar, r83 r83Var) {
        this.f116009a = context;
        this.f116010b = relativeLayout;
        this.f116011c = q2Var;
        this.f116012d = window;
        this.f116013e = str;
        this.f116014f = u3Var;
        this.f116015g = linearLayout;
        this.f116016h = textView;
        this.f116017i = progressBar;
        this.f116018j = r83Var;
    }

    public static final void b(t1 t1Var, View view) {
        ((q2) t1Var.f116011c).f114851a.finish();
    }

    public final void a(ImageView imageView, ImageView imageView2) {
        imageView.setOnClickListener(new View.OnClickListener() { // from class: yads.qq0
            @Override // android.view.View.OnClickListener
            public final void onClick(View view) {
                t1.a(this.f115128b, view);
            }
        });
        imageView2.setOnClickListener(new View.OnClickListener() { // from class: yads.rq0
            @Override // android.view.View.OnClickListener
            public final void onClick(View view) {
                t1.b(this.f115555b, view);
            }
        });
    }

    @Override // yads.h2
    public final void c() {
        this.f116010b.setBackgroundDrawable(o9.f114226a);
        LinearLayout linearLayout = this.f116015g;
        Context context = this.f116009a;
        r9 r9Var = r9.f115371e;
        ImageView imageViewA = p9.a(context, "data:image/png;base64,", qi3.a(context, r9Var.a(context) - 2));
        Context context2 = this.f116009a;
        ImageView imageViewA2 = p9.a(context2, "data:image/png;base64,", qi3.a(context2, r9Var.a(context2)));
        imageViewA2.setTag(ni3.a("browser_close_button"));
        a(imageViewA, imageViewA2);
        linearLayout.addView(this.f116016h, new LinearLayout.LayoutParams(0, -1, 1.0f));
        linearLayout.addView(imageViewA, new LinearLayout.LayoutParams(-2, -1));
        linearLayout.addView(imageViewA2, new LinearLayout.LayoutParams(-2, -1));
        RelativeLayout relativeLayout = this.f116010b;
        LinearLayout linearLayout2 = this.f116015g;
        RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams(-1, qi3.a(this.f116009a, r9.f115370d.a(r2)));
        layoutParams.addRule(10);
        relativeLayout.addView(linearLayout2, layoutParams);
        Context context3 = this.f116009a;
        LinearLayout linearLayout3 = this.f116015g;
        RelativeLayout.LayoutParams layoutParams2 = new RelativeLayout.LayoutParams(-1, qi3.a(context3, 2.0f));
        layoutParams2.addRule(3, linearLayout3.getId());
        this.f116010b.addView(this.f116017i, layoutParams2);
        a(8);
        RelativeLayout relativeLayout2 = this.f116010b;
        u3 u3Var = this.f116014f;
        LinearLayout linearLayout4 = this.f116015g;
        RelativeLayout.LayoutParams layoutParams3 = new RelativeLayout.LayoutParams(-1, -1);
        layoutParams3.addRule(3, linearLayout4.getId());
        relativeLayout2.addView(u3Var, layoutParams3);
        this.f116014f.loadUrl(this.f116013e);
        ((q2) this.f116011c).a(6, null);
        boolean z10 = lb1.f113032a;
    }

    @Override // yads.h2
    public final boolean d() {
        boolean z10;
        if (this.f116014f.canGoBack()) {
            u3 u3Var = this.f116014f;
            if (u3Var.canGoBack()) {
                u3Var.goBack();
            }
            z10 = true;
        } else {
            z10 = false;
        }
        return !z10;
    }

    @Override // yads.h2
    public final void f() {
        this.f116012d.requestFeature(1);
        if (sb.a(16)) {
            this.f116012d.getDecorView().setSystemUiVisibility(256);
        }
    }

    @Override // yads.h2
    public final void g() {
        this.f116014f.c();
    }

    @Override // yads.h2
    public final void onAdClosed() {
        ((q2) this.f116011c).a(8, null);
    }

    public static final void a(t1 t1Var, View view) {
        String url = t1Var.f116014f.getUrl();
        if (url != null) {
            t1Var.f116018j.a(t1Var.f116009a, url);
        }
    }

    @Override // yads.h2
    public final void b() {
        u3 u3Var = this.f116014f;
        u3Var.getClass();
        int i10 = rb.f115393b;
        try {
            WebView.class.getDeclaredMethod(C4240b4.i.f42653t0, null).invoke(u3Var, null);
        } catch (Exception unused) {
        }
        u3.class.toString();
        boolean z10 = lb1.f113032a;
    }

    @Override // yads.h2
    public final void a() {
        u3 u3Var = this.f116014f;
        u3Var.getClass();
        int i10 = rb.f115393b;
        try {
            WebView.class.getDeclaredMethod(C4240b4.i.f42655u0, null).invoke(u3Var, null);
        } catch (Exception unused) {
        }
        u3.class.toString();
        boolean z10 = lb1.f113032a;
    }

    public final void a(int i10) {
        if (i10 == 0 && this.f116017i.getVisibility() != 0) {
            this.f116017i.bringToFront();
            this.f116010b.requestLayout();
            this.f116010b.invalidate();
        }
        this.f116017i.setVisibility(i10);
    }
}
