package b9;

import android.content.Context;
import android.view.View;

/* JADX INFO: loaded from: classes12.dex */
public class n extends l {

    /* JADX INFO: renamed from: g, reason: collision with root package name */
    private boolean f5927g;

    public n(View.OnClickListener onClickListener) {
        super(onClickListener);
        this.f5927g = false;
    }

    @Override // b9.l
    protected k j(Context context, k kVar) {
        if (kVar == null || !"speakerfill".equals(kVar.D())) {
            return a.f5843g;
        }
        k kVar2 = new k();
        kVar2.X(Boolean.TRUE);
        return a.f5843g.g(kVar2);
    }

    /* JADX INFO: Access modifiers changed from: protected */
    @Override // b9.l
    /* JADX INFO: renamed from: r, reason: merged with bridge method [inline-methods] */
    public void c(Context context, f9.a aVar, k kVar) {
        aVar.setImage(a.a(this.f5927g ? "" : ""));
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    @Override // b9.l
    /* JADX INFO: renamed from: s, reason: merged with bridge method [inline-methods] */
    public f9.a h(Context context, k kVar) {
        return new f9.a(context);
    }

    public void t(boolean z10) {
        this.f5927g = z10;
        m();
    }
}
