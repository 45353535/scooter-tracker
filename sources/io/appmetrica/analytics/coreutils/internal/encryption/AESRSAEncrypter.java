...
public class AESRSAEncrypter implements Encrypter {
    ...
    final byte[] a(byte[] bArr, byte[] bArr2, byte[] bArr3, PublicKey publicKey) {
        ...
        try {
            ...
            Cipher cipher = Cipher.getInstance(this.f75258b);
            cipher.init(1, publicKey);
            byteArrayOutputStream = new ByteArrayOutputStream(bArr.length);
            try {
                byteArrayOutputStream.write(cipher.doFinal(byteArray));
                byte[] bArrEncrypt = new AESEncrypter(this.f75257a, bArr2, bArr3).encrypt(bArr);
                if (bArrEncrypt != null) {
    ...
    AESRSAEncrypter(int i10) {
        this.f75257a = AESEncrypter.DEFAULT_ALGORITHM;
        this.f75258b = C4228aa.f42351b;
    }
}
